...
(defun my-reposition-window ()
  ...
  (if (derived-mode-p 'fundamental-mode 'dired-mode 'Man-mode 'conf-mode 'mail-archives-mode)
      (recenter-top)
    (reposition-window)))
...
;; Can't add this to bindings.el because ‘C-right’ as ‘right-word’
;; or ‘right-sexp’ is often used after ‘C-x C-right’ (‘next-buffer’):
