...
(push '(display-buffer-from-next-error-p display-buffer-same-window)
      display-buffer-alist)
#+end_src
...
#+begin_src emacs-lisp
(setq-default repeat-exit-key [return])
(repeat-mode +1)
...
;; Can't add this to bindings.el because `C-right' as `right-sexp'
;; is often used after `C-x C-right' (`next-buffer'):
