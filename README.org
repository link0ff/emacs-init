...
(advice-add 'xref-goto-xref :around
            (lambda (orig-fun &rest args)
...
Flyspell only on moving point from the edited word,
not when moving point without editing:

#+begin_src emacs-lisp
(defun my-flyspell-post-command-hook ()
  (when flyspell-mode
    (with-local-quit
...
(add-hook 'flyspell-mode-hook
          (lambda ()
            ...
            (add-hook 'post-command-hook (function my-flyspell-post-command-hook) t t)))
#+end_src
  ...
  (advice-add 'project-find-regexp :around
              (lambda (orig-fun &rest args)
                ...
                (let ((xref-buffer-name
                  ...
                  (apply orig-fun args)))
              '((name . project-find-regexp-unique))))
#+end_src
