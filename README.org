...
(defun my-reposition-window ()
  ...
  (if (memq major-mode '(fundamental-mode dired-mode Man-mode))
      (recenter-top)
    (reposition-window)))
...
Then a key sequence =M-right M-left M-down ...= can be used to quickly inspect
...
(setq isearch-push-state-function
      (lambda ()
        ...
        (when (and isearch-success (not (pos-visible-in-window-p)))
          ...
          (if (or (memq major-mode '(fundamental-mode dired-mode Man-mode))
                  (> (buffer-size) 1000000))
              (recenter-top)
            (condition-case nil
