...
~outline-cycle-minor-mode~ and use =TAB= keys with modifiers to cycle visibility.
...
to enable ~outline-cycle-highlight-minor-mode~ to highlight outline headings.
...
#+begin_src emacs-lisp
;; TODO: add xref hook?
(advice-add 'xref--insert-xrefs :after
            (lambda (&rest _args)
              (setq-local outline-regexp "/")
              (outline-cycle-highlight-minor-mode +1))
            '((name . xref-outline-cycle-highlight-minor-mode)))
#+end_src
...
(with-eval-after-load 'man
  ...
  (add-hook 'Man-cooked-hook 'outline-cycle-highlight-minor-mode)
  ;; Mozilla-like navigation:
