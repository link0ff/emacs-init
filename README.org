...
(defun my-reposition-window ()
  ...
  ;; `reposition-window' on a newly visited file takes too much time,
  ;; and even `pos-visible-in-window-p' causes `jit-lock' calls.
...
;; (keymap-set outline-inserted-button-map "/ s" #'outline-show-by-heading-regexp)
;; (keymap-set outline-inserted-button-map "/ h" #'outline-hide-by-heading-regexp)
#+end_src
...
(add-hook 'archive-extract-hook 'my-outline-minor-mode)
...
(defun my-hs-minor-mode ()
  (when (or (and (treesit-parser-list)
                 ...
                 (not (derived-mode-p '(liquid-generic-ts-mode)))) ; liquid only for big files
            (derived-mode-p '(emacs-lisp-mode)))
    (hs-minor-mode +1)))

;; Moving this hook to end changes the order of overlays,
;; but not order of indicators in margins:
(add-hook 'find-file-hook 'my-hs-minor-mode) ;; 90
(add-hook 'archive-extract-hook 'my-hs-minor-mode)
...
(keymap-set global-map "TAB" #'my-indent-or-complete)
#+end_src
...
(put 'archive-previous-line 'isearch-motion '(left-char . backward))
(put 'archive-next-line 'isearch-motion '(right-char . forward))
      ...
      (lambda ()
        ...
        (when (and isearch-success
          ...
          (if (or (derived-mode-p '( fundamental-mode dired-mode Man-mode org-mode
                                     markdown-mode conf-mode sh-mode bash-ts-mode c-ts-mode))
                  (> (buffer-size) 1000000))
              (recenter-top)
            (condition-case nil
...
(defun my-enable-flyspell ()
  (unless (or flyspell-mode                         ;; already enabled
    ...
    ;; Postpone to not redisplay in the middle of the existing redisplay
    (letrec ((postfun
              (lambda ()
...
Some XML files don't begin with mandatory ~<?xml version="1.0"?>~,
so add more heuristics:

#+begin_src emacs-lisp
(push '("<[-[:alnum:]]+ xmlns=" . xml-mode) magic-fallback-mode-alist)
#+end_src
...
(with-eval-after-load 'log-view
  (keymap-set log-view-mode-map "TAB"
              `(menu-item
                "" log-view-msg-next
                :filter ,(lambda (_cmd)
                           (if (or (save-excursion
                               ...
                               #'log-view-msg-next
                             #'forward-button))))
  (keymap-set log-view-mode-map "<backtab>" ;; "S-TAB"
              `(menu-item
                "" log-view-msg-prev
                :filter ,(lambda (_cmd)
                           (if (or (save-excursion
...
with the current working tree (bug#35624, bug#35860, bug#62940):
...
Move point to the editable field in the Customization buffer (bug#80218):

#+begin_src emacs-lisp
(define-advice custom-buffer-create-internal (:after (&rest _args) navigate)
  (when (search-forward "Hide" nil t)
    (widget-forward 1)))
#+end_src
...
;; (define-key gnus-summary-mode-map "M-s" 'gnus-summary-repeat-search-article-forward)
  ...
  (add-hook 'gnus-summary-mode-hook
            (lambda ()
              ;; Don't split the narrow Summary buffer
...
(setopt hi-lock-face-defaults
        (append
         ;; Some default faces that are intense and vivid
         '("hi-yellow" "hi-salmon" "hi-aquamarine")
         ;; More faces with rich colors
         (mapcar (lambda (color)
         ...
         ;; Remaining default faces that are too washed-out
         ;; and too similar to diff/refined background colors
         '("hi-pink" "hi-green" "hi-blue")))
#+end_src
...
(defun project-any-transient (dir)
  (when (memq this-command '(project-find-regexp))
    `(transient . ,dir)))
(add-hook 'project-find-functions 'project-any-transient 20)

;; But don't remember transient projects:
(add-to-list 'project-list-exclude (lambda (pr) (eq (car pr) 'transient)))
#+end_src

Also exclude remote projects from remembering:

#+begin_src emacs-lisp
(add-to-list 'project-list-exclude
             (lambda (pr)
               (and (eq (nth 0 pr) 'vc)
                    (file-remote-p (nth 2 pr)))))
#+end_src

Also don't check remote dirs for project roots:

#+begin_src emacs-lisp
(defun project-disable-remote (dir)
  (when (file-remote-p dir)
    ;; Unfortunately, this writes log message: "Cannot access file or directory".
    (signal 'permission-denied (list (format "Remote projects disabled in %S" dir)))))
(add-hook 'project-find-functions 'project-disable-remote -20)
#+end_src

The setting above allows using project uniquification
and disable it on remote dirs:

#+begin_src emacs-lisp
(setopt uniquify-dirname-transform #'project-uniquify-dirname-transform)
#+end_src
