...
It's useful to run such key sequences =C-x p p C-x v L=
from any buffer:

#+begin_src emacs-lisp
(with-eval-after-load 'vc
  (setq vc-deduce-backend-nonvc-modes nil))
#+end_src
...
#+begin_src emacs-lisp
(add-hook 'next-error-hook (lambda () (when outline-minor-mode (outline-show-entry))))
#+end_src
...
(define-advice read-directory-name (:around (ofun prompt &optional dir default-dirname mustmatch initial) project-defaults)
