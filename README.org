...
(define-advice make-frame-on-monitor (:around (ofun monitor &optional display parameters)
                                      make-frame-on-monitor-undecorated)
  (funcall ofun monitor display (append '((undecorated . t)) parameters)))

(define-advice make-frame-on-current-monitor (:around (ofun &optional parameters)
...
#+begin_src emacs-lisp
(define-advice gui-selection-value (:around (ofun &rest args) gui-selection-value-url-decode)
...
(define-advice dired-backup-diff (:around (ofun &rest args) my-dired-backup-diff)
...
;; Get coding from the file, so diff will output in the correct coding:
(define-advice dired-diff (:around (ofun &rest args) my-dired-diff)
  (let* ((filename (dired-get-filename))
         (coding-system (when (and (file-regular-p filename)
                                   (not (file-remote-p filename)))
                          (with-temp-buffer
...
#+begin_src emacs-lisp
(define-advice lgrep (:around (ofun &rest args) vc-git-grep-no-history)
  (let ((grep-history '()))
    (apply ofun args)))
(define-advice vc-git-grep (:around (ofun &rest args) vc-git-grep-no-history)
  (let ((grep-history '()))
    (apply ofun args)))
#+end_src
...
#+begin_src emacs-lisp
(define-advice isearch-mode (:around (ofun forward &optional regexp &rest args) isearch-mode-regexp-after-left-word)
...
(with-eval-after-load 'project
  ...
  (define-advice project-find-regexp (:around (ofun &rest args) project-find-regexp-unique)
    (require 'xref)
...
(define-advice vc-git-diff (:around (ofun &rest args) vc-git-diff-use-orderfile)
...
#+begin_src emacs-lisp
(define-advice diff-backup (:around (ofun file &optional switches) diff-backup-fallback-to-vc-diff)
  (condition-case err
...
(define-advice vc-diff (:around (ofun &rest args) vc-diff-fallback-to-diff-backup)
...
(define-advice read-directory-name (:around (ofun prompt &optional dir default-dirname
			                          mustmatch initial)
  ...
  (let ((ret (funcall ofun prompt dir default-dirname
		      mustmatch initial)))
    (when (featurep 'project)
