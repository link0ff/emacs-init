...
(let ((map outline-minor-mode-cycle-map)
...
(setq-default
 ...
 windmove-display-default-keybindings '(nil hyper meta)
...
(push '(display-buffer-from-next-error-p
        display-buffer-same-window
        (inhibit-same-window . nil))
      display-buffer-alist)
#+end_src
...
Unset ~repeat-exit-timeout~ in the debugger buffers
where the repeat key can be pressed after a long delay.

#+begin_src emacs-lisp
(advice-add 'gud-find-file :around
            (lambda (orig-fun &rest args)
              (let ((buf (apply orig-fun args)))
                (when (buffer-live-p buf)
                  (with-current-buffer buf
                    (setq-local repeat-exit-timeout nil)))
                buf))
            '((name . disable-repeat-timeout-in-gdb)))
#+end_src
...
(advice-add 'add-to-history :around
            (lambda (orig-fun history-var newelt &rest args)
              (unless (or (and (memq history-var '(shell-command-history compile-history))
...
#+begin_src emacs-lisp
(push `(,(rx bos "*" (and "input/output of " (+ (any "A-za-z"))) "*" eos)
...
(push '("\\`\\*\\(Org \\(Links\\|Select\\)\\|Org-Babel Error Output\\)\\*\\'"
        display-buffer-below-selected
