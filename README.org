...
#+begin_src emacs-lisp
;; TODO: maybe move to ‘display-buffer’ call in ‘display-warning’
;; ALSO bug in lib that (warn "Test") doesn't move point to the bottom of the window
;; when window is already displayed
(add-to-list 'display-buffer-alist
             `(,(rx bos "*Warnings*" eos)
               (display-buffer-reuse-window
