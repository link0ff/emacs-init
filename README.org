...
- [[#space-optimization][Space optimization]]
  ...
  - [[#org-like-s-tab-heading-cycle-in-all-file-buffers][Org-like (S-)TAB heading cycle in all file buffers]]
...
** Org-like (S-)TAB heading cycle in all file buffers
...
~outline-minor-mode~ and use =TAB= keys with modifiers to cycle visibility.
...
In some modes that use not too much highlighting, it's possible also
to enable ~outline-minor-mode-highlight~ to highlight outline headings.
...
(defun my-outline-minor-mode ()
  (unless (derived-mode-p 'outline-mode)
    (outline-cycle-minor-mode +1)))
(add-hook 'find-file-hooks 'my-outline-minor-mode)
#+end_src

Also enable cycling on filename headings in *xref* buffers:

#+begin_src emacs-lisp
(advice-add 'xref--show-xref-buffer :around
            (lambda (orig-fun &rest args)
              (with-current-buffer (apply orig-fun args)
                (setq-local outline-regexp "/")
                (outline-cycle-minor-mode +1)))
            '((name . xref-outline-cycle-minor-mode)))
#+end_src
...
=C-RET= in the =*Completions*= buffer inserts the selected completion
to the minibuffer without exiting it.
...
;; TODO: Maybe better to add a hook dictionary-search-post-hook?
(with-eval-after-load 'dictionary
  (add-to-list 'debug-ignored-errors "^No match for \"[^\"]+\" with strategy \"[^\"]+\" in dictionary")
  (advice-add 'dictionary-post-buffer :after
              (lambda ()
                (goto-char (point-max))
                (recenter -2 t))
              '((name . dictionary-post-buffer-recenter))))
#+end_src
...
(define-key xref--xref-buffer-mode-map "\t" 'xref-next-line-no-display)
(define-key xref--xref-buffer-mode-map [backtab] 'xref-prev-line-no-display)
...
(with-eval-after-load 'org-faces
  ;; Header color like in Wikipedia
