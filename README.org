...
#+begin_src emacs-lisp
(define-advice beginning-of-defun (:around (orig-fun &rest args) recenter-top)
...
(define-advice make-frame-on-monitor (:around (orig-fun monitor &optional display parameters)
                                      make-frame-on-monitor-undecorated)
  (funcall orig-fun monitor display (append '((undecorated . t)) parameters)))

(define-advice make-frame-on-current-monitor (:around (orig-fun &optional parameters)
                                              make-frame-on-current-monitor-undecorated)
...
(defun my-outline-minor-mode ()
  ...
  (when (derived-mode-p 'emacs-lisp-mode 'ruby-mode 'ruby-ts-mode)
...
(define-advice xref-goto-xref (:around
                               (orig-fun &rest args)
                               window--display-buffer-set-previous-window)
  (let ((buffer (current-buffer)))
    (apply orig-fun args)
    (with-current-buffer buffer
      (setq-local display-buffer-previous-window (selected-window)))))

;; (define-advice window--display-buffer (:around
;;                                        (orig-fun &rest args)
;;                                        window--display-buffer-set-previous-window)
;;   (let ((buffer (current-buffer))
...
#+begin_src emacs-lisp
;; (define-advice edebug-pop-to-buffer (:around
;;                                      (orig-fun buffer &optional window)
;;                                      edebug-pop-to-buffer-same-window)
...
#+begin_src emacs-lisp
(define-advice beginning-of-buffer-other-window (:after (&rest _args)
                                                        beginning-of-buffer-other-window-recenter)
  (with-selected-window (other-window-for-scrolling)
    (recenter-top-bottom)))

(define-advice end-of-buffer-other-window (:after (&rest _args)
                                                  end-of-buffer-other-window-recenter)
  (with-selected-window (other-window-for-scrolling)
    (recenter -1)))
#+end_src
...
#+begin_src emacs-lisp
(define-advice gui-selection-value (:around (orig-fun &rest args) gui-selection-value-url-decode)
...
(setq isearch-push-state-function
      (lambda ()
        ...
        (when (and isearch-success
          ...
          (if (or (derived-mode-p 'fundamental-mode 'dired-mode 'Man-mode 'markdown-mode
                                  'conf-mode 'sh-mode 'bash-ts-mode)
                  (> (buffer-size) 1000000))
              (recenter-top)
            (condition-case nil
...
#+begin_src emacs-lisp
(define-advice add-to-history (:around (orig-fun history-var newelt &rest args)
                                       add-to-history-ignorespace)
...
(define-advice fill-paragraph (:before (&rest _args) fill-paragraph-double-space)
  (when (use-region-p)
...
#+begin_src emacs-lisp
(define-advice vc-git-log-search (:after (buffer pattern) vc-git-log-search-highlight)
  (with-current-buffer buffer
    (when pattern
      (vc-run-delayed
...
(define-advice dired-backup-diff (:around (orig-fun &rest args) my-dired-backup-diff)
...
;; Get coding from the file, so diff will output in the correct coding:
(define-advice dired-diff (:around (orig-fun &rest args) my-dired-diff)
  (let* ((filename (dired-get-filename))
         (coding-system (when (and (file-regular-p filename)
                                   (not (file-remote-p filename)))
                          (with-temp-buffer
...
#+begin_src emacs-lisp
(define-advice lgrep (:around (orig-fun &rest args) vc-git-grep-no-history)
  (let ((grep-history '()))
    (apply orig-fun args)))
(define-advice vc-git-grep (:around (orig-fun &rest args) vc-git-grep-no-history)
  (let ((grep-history '()))
    (apply orig-fun args)))
#+end_src
...
#+begin_src emacs-lisp
(define-advice base64-decode-region (:after (beg end &rest _) base64-decode-region-with-buffer-coding)
...
(define-advice right-word (:around (orig-fun &rest args) mixed-content)
  (if (right-script-p)
      (apply #'backward-word args)
    (apply orig-fun args)))

(define-advice left-word (:around (orig-fun &rest args) mixed-content)
  (if (right-script-p)
...
#+begin_src emacs-lisp
(define-advice isearch-mode (:around (orig-fun forward &optional regexp &rest args) isearch-mode-regexp-after-left-word)
...
(with-eval-after-load 'xref
  ...
  (define-advice xref-find-definitions (:after (&rest _args) from-xref-find-definitions)
    (with-current-buffer (window-buffer)
...
(with-eval-after-load 'project
  ...
  (define-advice project-find-regexp (:around (orig-fun &rest args) project-find-regexp-unique)
    (require 'xref)
...
#+begin_src emacs-lisp
(define-advice set-variable (:around (orig-fun &rest args) override-custom-variable)
  (interactive (lambda (spec)
...
(define-advice vc-dir-deduce-fileset (:after (&rest _) vc-dir-create-orderfile)
...
(define-advice vc-git-diff (:around (orig-fun &rest args) vc-git-diff-use-orderfile)
...
#+begin_src emacs-lisp
(define-advice diff-backup (:around (orig-fun file &optional switches) diff-backup-fallback-to-vc-diff)
  (condition-case err
...
(define-advice vc-diff (:around (orig-fun &rest args) vc-diff-fallback-to-diff-backup)
...
(define-advice read-directory-name (:around (orig-fun prompt &optional dir default-dirname
			                          mustmatch initial)
                                            read-directory-name-project-defaults)
...
(define-advice vc-dir (:after (dir &optional _backend) vc-dir-add-project)
  (when (featurep 'project)
