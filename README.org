...
;; The old way to enable flyspell that causes too much ispell process restarts
;; when loading the desktop with many files that use different personal dictionaries.
;; It was also enabled by dir-local settings '(mode . flyspell)'.
...
;;   ;; and so restarts the Ispell process (bug#69703).
...
Also allow adding the file ‘.project’ as the project root marker:
